...
class IKeypoint(nn.Module):
    ...
    def __init__(self,
                 ...
                 ch=(),
                 inplace=True,
                 dw_conv_kpt=False):  # detection layer
        ...
        self.m = nn.ModuleList(
            nn.Conv2d(x, self.no_det * self.na, 1) for x in ch)  # output conv
            ...
            if self.dw_conv_kpt:  #keypoint head is slightly more complex
                self.m_kpt = nn.ModuleList(
                    nn.Sequential(
                        ...
                        Conv(x, x), DWConv(x, x, k=3), Conv(x, x),
                        DWConv(x, x, k=3), Conv(x, x), DWConv(x, x, k=3),
                        Conv(x, x), DWConv(x, x, k=3),
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            if self.nkpt is None or self.nkpt == 0:
                ...
                x[i] = torch.cat((self.im[i](self.m[i](self.ia[i](
            ...
            if not self.training:  # inference
                ...
                if self.inplace:
                    ...
                    if self.nkpt != 0:
                        y[...,
                          6:] = (y[..., 6:] * 2. - 0.5 + self.grid[i].repeat(
...
class IAuxDetect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.im = nn.ModuleList(
            ImplicitM(self.no * self.na) for _ in ch[:self.nl])
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x[i + self.nl] = x[i + self.nl].view(bs, self.na, self.no, ny,
                                                 nx).permute(0, 1, 3, 4,
                                                             2).contiguous()
...
class IBin(nn.Module):
    ...
    def __init__(self,
                 ...
                 anchors=(),
                 ch=(),
                 bin_count=21):  # detection layer
        super(IBin, self).__init__()
        self.nc = nc  # number of classes
        self.bin_count = bin_count

        self.w_bin_sigmoid = SigmoidBin(bin_count=self.bin_count,
                                        min=0.0,
                                        max=4.0)
        self.h_bin_sigmoid = SigmoidBin(bin_count=self.bin_count,
                                        min=0.0,
                                        max=4.0)
        # classes, x,y,obj
        self.no = nc + 3 + \
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(
                        m, IDetect) or isinstance(m, IAuxDetect) or isinstance(
                            m, IKeypoint):
                    break
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m,
        ...
        if m in [
                ...
                RepConv, RepConv_OREPA, DownC, SPP, SPPF, SPPCSPC,
                ...
                ResXCSPC, RepResX, RepResXCSPA, RepResXCSPB, RepResXCSPC,
                Ghost, GhostCSPA, GhostCSPB, GhostCSPC, SwinTransformerBlock,
                STCSPA, STCSPB, STCSPC, SwinTransformer2Block, ST2CSPA,
                ST2CSPB, ST2CSPC
        ]:
            c1, c2 = ch[f], args[0]
            ...
            args = [c1, c2, *args[1:]]
            if m in [
                    DownC, SPPCSPC, GhostSPPCSPC, BottleneckCSPA,
